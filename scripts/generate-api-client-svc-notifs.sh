...
# Usage: 
# ./scripts/generate-api-client-svc-notifications.sh -e staging -u <USERNAME_HERE> -p <PASSWORD_HERE>
...
AELER_URL_BASE="https://staging.aeler.com/nc/api"
...
jq -r '. + {"securityDefinitions": {"jwt": {"type": "apiKey", "name": "Authorization", "in": "header"}}, "security": [{"jwt": []}]}' <<< $HTTP_BODY > ./scripts/spec-notifs.json

##############################################################################

# curl ${server}/documentation?tags=${tags} | jq -r '. + {"securityDefinitions": {"jwt": {"type": "apiKey", "name": "Authorization", "in": "header"}}, "security": [{"jwt": []}]}' > ./scripts/spec-notifs.json
sed -i -e 's/"format": "date"/"format": "date-time"/g' ./scripts/spec-notifs.json
sed -i -e 's/"readAt": {/"readAt": {"x-nullable": true,/g' ./scripts/spec-notifs.json

docker run --rm -v ${PWD}:/local --network="host" -u `id -u $USER` openapitools/openapi-generator-cli:v5.1.0 generate \
  -i /local/scripts/spec-notifs.json \
  -g typescript-fetch \
  -o /local/src/apis-client/svc-notifs \
